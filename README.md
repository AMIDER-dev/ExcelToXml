...
### 要素名定義テーブル
データテーブルの要素名とサンプルXMLの各XPathの対応付けを定義する。要素名の階層構造はスラッシュ（/）で表す。繰り返される可能性のあるタグに対しては、そのタグを末尾とするXPath＆要素名を定義しておく。すなわち、ある要素の繰り返しがあり、かつ繰り返される起点がその要素の上位タグの場合、そのタグ名も親要素として定義する必要がある。
